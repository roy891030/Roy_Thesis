...
在上述脈絡下，結合多因子表示（multi-factor representation）與圖關注（graph attention）的模型架構，特別是能分離產業與全市場共同影響的設計，提供了一條兼顧「特徵可學習性」與「關係結構建模」的路徑。本文後續將採用深度多因子模型結合圖關注之框架，並以台灣日頻資料進行實證檢驗。
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[component, below=10mm of ch1, xshift=-3.7cm] (b1) {線性迴歸在股票報酬預測中的角色與限制\\（Linear Regression）};
        \node[component, right=10mm of b1] (b2) {非線性機器學習模型\\與特徵工程導向方法\\（Tree-based Models）};
        \node[component, below=7mm of b1] (b3) {技術指標之特徵表示};
        \node[component, below=7mm of b2] (b4) {時間序列模型在金融預測中的應用\\（LSTM）};
        \node[component, below=7mm of b3] (b5) {圖神經網路的基本概念與代表性架構\\（以 GAT 為代表）};
        \node[component, below=7mm of b4] (b6) {GAT 在股票預測與排序任務上\\的應用與限制};
        \node[componentwide, below=7mm of b5, xshift=3.7cm] (b7) {IC 與 ICIR 的理論意涵與實務角色};
